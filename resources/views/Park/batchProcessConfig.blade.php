...
<div class="col-md-8">
  <div class="ibox ">
    <div class="ibox-title bg-title">
      <h5>Batch Data Processing</h5>
      <div class="ibox-tools">
      </div>
    </div>
    <div class="ibox-content">
      <div>
        <form method="post" action="{{ action('ParkController@batchProcessSend') }}" class="form-horizontal">
          @csrf
          <script type="text/javascript">

              window.onload = function () {
                var loadData = angular.element(document.getElementById('StreamConfigCtrl')).scope().getCookieSetting2("batchSetting2");
                angular.element(document.getElementById('StreamConfigCtrl')).scope().getCookieSetting1("batchSetting1");
          ...
          <div class="form-group"><label class="col-sm-2 control-label text-left"> Batch Data Source </label>
            <div class="col-sm-10"><input class="form-control" type="text" name="dataSource" required value="@{{ input.batchData }}"></div>
          </div>
          <div class="hr-line-dashed"></div>
          <div class="form-group"><label class="col-sm-2 control-label text-left"> Batch Data Destination </label>
            <div class="col-sm-10"><input class="form-control" type="text" name="dataDestination" required value="@{{ input.savePath }}">
            </div>
          </div>
          <div class="hr-line-dashed"></div>
...
"mqtt-payload":"This is TEMPERATURE payload"
          ...
          <div class="form-group">
            <div class="col-sm-12" style="height: 30px;">
                <button ng-click="addSensor()" type="button" class="btn-primary form-btn"  >
                  Add
                </button>
            </div>
          </div>
...
<script src="{{ asset('js/customJS/streamDataConfig/ang.push.data.js?v=1AB001234') }}" charset="utf-8"></script>
 ...
 var cm = new Array();
  ...
  function saveFormData(){
     ...
     var input = new Object();
          ...
          input.batchData = $("input[name='dataSource']").val();
          input.savePath = $("input[name='dataDestination']").val();

      var formData = JSON.stringify(input);
      setCookie('batchSetting1', formData,10);
      ...
      if (cm != null && cm.length > 0){
        ...
        setCookie('cm', searchfor.length,30);
      }

      if (searchfor.length > 0 ){
        var formData2 = JSON.stringify( searchfor );
        setCookie('batchSetting2', formData2, 30);
      }
